...
sprintf('<p>In %s, at the end of %s, %s%% of urban residents live within a 300m walk of a physically protected bikeway. More generally, %s%% of residents lived within a 300m walk of any bikeway, protected or unprotected. In %s, in %s, there were %s kilometers of bikeways, of which %s were physically protected. %s</p>', 
        ifelse(length(rank$indicator$name) == 0, "", style(unique(rank$indicator$name))), 
        style(input$year), 
        ifelse(length(format_indicator_value) == 0, "", style(unique(format_indicator_value))), 
        ifelse(length(rank$indicator$bike_pnpbpnab) == 0, "", style(round(rank$indicator$bike_pnpbpnab * 100))), 
        style(unique(rank$indicator$name)), 
        style(input$year), 
        ifelse(length(rank$indicator$bike_pnpbabikeways) == 0, "", style(round(rank$indicator$bike_pnpbabikeways))),
        ifelse(length(rank$indicator$bike_pnpbpbikeways) == 0, "", style(round(rank$indicator$bike_pnpbpbikeways))),
...
'<p>Studies have shown that <a href="https://jenniferdill.net/types-of-cyclists/">most</a> people would ride a bicycle for transport if it felt safe, but they choose not to because the existing streets feel too dangerous. In cities that provide extensive, well-connected networks of physically protected bikeways, large numbers of people cycle to get around. Increased cycling saves individuals time and money and <a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC9453587/">improves their health</a>. It also reduces air and noise pollution, carbon emissions, and <a href="https://www.heatwalkingcycling.org/">healthcare costs</a>. Protected bikeways <a href="https://www.rff.org/publications/journal-articles/bicycle-infrastructure-and-traffic-congestion-evidence-from-dcs-capital-bikeshare/">reduce congestion</a>, promote <a href="https://www.fastcompany.com/3021074/making-the-economic-case-for-cycling-friendly-cities-with-bikeonomics">local economic development</a>, and make streets safer and more pleasant, not only for cyclists but also for <a href="https://www.sciencedaily.com/releases/2019/05/190529113036.htm">motorists and pedestrians</a>.</p>',
...
'<li><strong>Develop the institutions to support and sustain cycling. </strong>Ensure lanes are well-designed, well-maintained, and integrated into ongoing city service provisions. Refer to specific modules in the ITDP’s <a href="https://itdp.org/course/mastering-the-cycling-city/">Mastering the Cycling City</a> course, such as “Design it” and “Institutionalize it,” for detailed insights.',
'</div>'
)
