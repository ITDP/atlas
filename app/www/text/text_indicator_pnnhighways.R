...
sprintf('<p>In %s in %s, %s%% of people lived at least 500m from a grade-separated highway. There were about %skm of highways in this area.</p>', 
        # 
        style(rank$indicator$name), 
        style(input$year), 
        style(format_indicator_value),
        style(ifelse(is.null(rank$indicator$walk_pnnhighwayskm), "X",round(rank$indicator$walk_pnnhighwayskm)))
        ), 
# style(rank$indicator$name), 
