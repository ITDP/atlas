...
list_folders <- dir("data-raw/data_nov18/city_results", full.names = TRUE)
# identify the code
...
prep_data <- function(ghsl) {
  ...
  ind_columns <- gsub(pattern = "(all_bikeways_km)_(\\d{4})",
                      replacement = "bike_abikeways_\\2",
                      x = ind_columns)
  ind_columns <- gsub(pattern = "(protected_bikeways_km)_(\\d{4})",
                      replacement = "bike_pbikeways_\\2",
                      x = ind_columns)
  ...
  dir.create(sprintf("data/sample5/ghsl_%s", ghsl), recursive = TRUE)
  
  # save
  readr::write_rds(data, sprintf("data/sample5/ghsl_%s/indicators_%s.rds", ghsl, ghsl))
...
purrr::walk(cities_available, prep_data)
...
prep_overlays <- function(ghsl) {
  ...
  base_dir <- sprintf("data-raw/data_nov18/city_results/ghsl_region_%s/", ghsl)
  
  # overlay files ------------------------------------
  overlay_files <- dir(paste0(base_dir, "geodata"), full.names = TRUE, pattern = ".geojson$", recursive = TRUE)
  
  # filter only our overlays
  overlay_files1 <- overlay_files[overlay_files %like% c("allbike_latlon|h\\+slatlon|healthcarelatlon|pnablatlon|pnpblatlon|pnftlatlon|protectedbike|schools")]
  ...
  save_by_ind <- function(ind) {
    ...
    dir.create(sprintf("data/sample5/ghsl_%s/overlays/%s", ghsl, ind), recursive = TRUE)
    ...
    if (nrow(overlay_lines) > 0) {
      ...
      readr::write_rds(overlay_lines_inds,    sprintf("data/sample5/ghsl_%s/overlays/%s/overlays_lines_%s_%s.rds", ghsl, ind, ghsl, ind))
    ...
    readr::write_rds(overlay_polygons_inds, sprintf("data/sample5/ghsl_%s/overlays/%s/overlays_polygons_%s_%s.rds", ghsl, ind, ghsl, ind))
  }
  ...
  readr::write_rds(overlay_df,       sprintf("data/sample5/ghsl_%s/overlays_%s.rds", ghsl, ghsl))
...
indicators_all %>% st_set_geometry(NULL) %>% 
  mutate(across(city_poptotal_1975:last_col(), round, 3)) %>%
...
# save
readr::write_rds(indicators_ghsl_centroids, "data/sample5/atlas_city_markers.rds")
...
atlas_country <- st_read("data-raw/data_nov18/country_results/country_results.geojson")
...
count(indicators_all_df, hdc, country, osmid, name, admin_level, admin_level_ordered) %>%
  select(-n) %>%
  readr::write_rds("data/sample5/list_osmid_name.rds")
