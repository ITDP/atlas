...
prep_overlays <- function(ghsl) {
  ...
  save_by_ind <- function(ind) {
    ...
    dir.create(sprintf("data/data_july2023/ghsl_%s/overlays/%s", ghsl, ind), recursive = TRUE)
    ...
    if (nrow(overlay_lines) > 0) {
      ...
      readr::write_rds(overlay_lines_inds,    sprintf("data/data_july2023/ghsl_%s/overlays/%s/overlays_lines_%s_%s.rds", ghsl, ind, ghsl, ind))
    ...
    readr::write_rds(overlay_polygons_inds, sprintf("data/data_july2023/ghsl_%s/overlays/%s/overlays_polygons_%s_%s.rds", ghsl, ind, ghsl, ind))
  }
...
atlas_country <- st_read("data-raw/data_july2023/country_results/country_results.geojson")
